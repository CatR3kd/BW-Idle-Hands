...
function doEvents(){
  ...
  if((eventHappening == false) && (nextEvent == 'penguinRequest') && (money >= 5_000_000) && (unlockedUpgrades.length >= 3)){
    ...
    const event = new Event(
      ...
      [
        {
          'text': `Accept the invitation${(localStorage.getItem('alliance') == 'penguin')? ' and betray the Penguins' : ''}`,
          'function': function(){
            event.text = 'We will be back soon. Avoid all communication with the penguins.';
            event.options = [{
            'text': 'Continue',
            'function': function(){
                eventHappening = false;
        ...
        {
          'text': 'Deny the invitation and anger the Pigeons',
          'function': function(){
            eventHappening = false;
            if(localStorage.getItem('alliance') == 'penguin'){
              localStorage.setItem('nextEvent', 'penguinPay');
